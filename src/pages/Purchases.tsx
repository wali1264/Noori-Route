...
export default function Purchases() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setFormData({ item: '', quantity: 1, unitPrice: 0, supplier: '' });
  };
  ...
  const generatePDF = () => {
    ...
    doc.text("Purchases Report - Noori Route ISP", 10, 10);
    ...
    (doc as any).autoTable({
      head: [['Item', 'Qty', 'Unit Price', 'Total', 'Supplier', 'Date']],
      body: tableData,
      startY: 20,
    });
    doc.save(`purchases-${new Date().toISOString().split('T')[0]}.pdf`);
  };
          ...
          <table className="w-full text-right text-xs">
            <thead className="bg-slate-50 text-slate-500 border-b border-slate-100 print:bg-white">
              <tr>
                ...
                <th className="px-6 py-4 font-bold">مجموع</th>
        ...
        {totalCount && totalCount > limit && !dateFilter.start && !dateFilter.end && (
          <div className="p-4 border-t border-slate-100 bg-slate-50/50 flex justify-center print:hidden">
            <button 
              ...
              className="flex items-center gap-2 px-8 py-2.5 rounded-full border border-slate-200 bg-white text-[10px] font-bold text-slate-600 hover:bg-slate-100 hover:border-orange-300 hover:text-orange-600 transition-all active:scale-95 shadow-sm"
            >
              مشاهده خریدات بیشتر
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="flex items-center justify-between mb-8">
                <h2 className="text-2xl font-bold text-slate-900">ثبت خرید جدید</h2>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  ...
                  <input
                    ...
                    className="mt-2 block w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 focus:border-orange-500 focus:bg-white focus:outline-none"
                    placeholder="مثلاً: پهنای باند انترنت"
                  />
                </div>
                <div className="grid gap-6 sm:grid-cols-2">
                  <div>
                    <label className="block text-sm font-bold text-slate-700">تعداد / مقدار</label>
                    <input
                      type="text"
                      inputMode="numeric"
                      required
                  ...
                  <div>
                    <label className="block text-sm font-bold text-slate-700">قیمت واحد (افغانی)</label>
                    <input
                      type="text"
                      inputMode="numeric"
                      required
                ...
                <div>
                  <label className="block text-sm font-bold text-slate-700">تأمین کننده (فروشنده)</label>
                  <input
                    type="text"
                    required
                ...
                <button
                  type="submit"
                  className="w-full rounded-xl bg-orange-600 py-4 text-lg font-bold text-white shadow-lg shadow-orange-200 transition-all hover:bg-orange-700 active:scale-95"
                >
                  ثبت نهایی
                </button>
              </form>
